...
//    List<NitnemEntry> findByName(String nitnemName);
    ...
    @Query("SELECT COALESCE(SUM(ne.completedCount), 0) FROM NitnemEntry ne WHERE ne.nitnem.id = :nitnemId")
    Integer getTillTodayCompletedCount(Long nitnemId);

    @Modifying
    @Transactional
    Integer deleteByNitnem(Nitnem nitnem);
}
